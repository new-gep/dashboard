		...
		return new Promise(async (resolve, reject) => {
			try {
				...
				if (response.status === 200) {
					...
					const dynamic = Object.values(response.date.dynamic.signature)
						// @ts-ignore
					...
					const responseSignature = await Signatures(
						manipulatingTable.cpf,
		...
		switch (action) {
			case 'observation':
				...
				const Newcandidates = candidates.map(
					//@ts-ignore
					({ cpf, step, status, verify, observation }) => ({
						cpf,
						step,
						status,
						verify,
				...
				const candidate = Newcandidates.find(
					(item: { cpf: string | undefined }) => item.cpf === manipulating.cpf,
				);
				if (candidate) {
					...
					{!loaderTable ? (
						...
						<>
							<table className='table table-modern' onClick={closeMenu}>
								...
								<tbody>
									{dataPagination(candidatesStep, currentPage, perPage).map(
										(item) => (
											<tr
												key={item.id}
												...
												<td>
													<Button
														...
														icon='Info'
														onClick={() =>
															handleUpcomingDetails(item.observation)
														}
